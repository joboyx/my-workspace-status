...
- TTY event loop: do not run git or other blocking I/O on the draw/event thread. Use `run_work_pumped`. Headless e2e may stay sync. Guard: `tty_event_loop_must_not_call_sync_pane_git` in `tui/event_pump.rs`.

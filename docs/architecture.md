...
The ratatui TUI applies crossterm `Resize` to `Terminal::resize` (the event size, not a later ioctl) and recomputes pane widths, graph gutter cap, help overlay rows, and list viewports from the new area. The event loop drains queued input with `poll(0)` + `read`, skips watch/fetch ticks while an overlay is open, and autoloads graph history only after list movement. Long git work (fetch / pull / push / default-branch / watch / full or focused reload **and** the follow-up graph `log` / file `diff` / commit files / commit diff) runs on a worker; the loop still paints and accepts resize and quit. An unchanged watch poll skips that pane reload so idle ticks do not re-run `git log`. TTY must not call sync `load_right`; Headless e2e uses `load_right_headless`. `tui/event_pump.rs` `tty_event_loop_must_not_call_sync_pane_git` fails CI if that split regresses.

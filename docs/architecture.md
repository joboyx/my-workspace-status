...
| -------------- | ----------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Config         | `config.rs`                                                                         | `WorkspaceStatusConfig` (`ignoredRepos`, `maxDepth`, `defaultBranches`, `editor`, `diffTool`)                                                                                 |
...
| Snapshot       | `discovery.rs` — `process_repo` via `parallel.rs` `map_with_concurrency` | `WorkspaceRepoSnapshot` per path (`checkoutKind`, `primaryRepo?`, `mergedIntoDefault`) from status + merge-base probe. Same-commit as the default tip is not merged. Cap `FETCH_CONCURRENCY` (10). |
...
`tui/watch.rs` polls file signatures (status + mtime) and tree chrome signatures (repo / checkout / dir / workspace / group, including `HEAD` and `sync_note`) so those rows flash on semantic updates, including remove ghosts for ~800ms. Graph list rows and commit-file rows use separate signature maps keyed by repo (and source). A disjoint identity set seeds and does not flash, so switching repos does not light up the whole graph. Fetch / pull / push / default-branch completion also flashes `repo:<path>` / `checkout:<path>` rows as `Update`. Add, update, and remove each use a four-step background ramp (`flash` / `flash_update` / `flash_remove`). Flash background wins over cursor and search. Status letter colours stay. Watch discover + per-checkout `process_repo` jobs share the same cap as fetch; each `RepoSnapshot` replaces that path immediately and the focused pane reloads when that checkout's identity moves. Identical signatures **and** unchanged checkout identities (`HEAD` / sync note / dirty set) skip that pane git. The next poll is scheduled from the start of the interval, not after collect finishes. Keys cannot starve the tick: the loop is a fair `select!` with a bounded input batch.

`tui/fetch.rs` sets the background fetch period (`WS_STATUS_FETCH_MS`, default 5 minutes; `0` disables) and `background_fetch_targets`. Manual `f` and `FetchTick` enqueue on the per-gitdir remote queue in `tui/effect.rs`. Occupy key is `primary_repo` when set, else the checkout path. Cap is `FETCH_CONCURRENCY` (10; `WS_STATUS_FETCH_CONCURRENCY`) on the Scheduler JoinSet. Progress is `Fetching n/N…` as completions land. Mixed kinds paint `Fetching 1 · Pulling 1…` or `Fetching 1/2 · queued 1`. File `row_signature` is status letter + `size:mtimeMs` (or `gone`), and `background_fetch_targets` is every snapshot except hidden ignored (linked worktrees included). Manual `f` stays on focus-scoped `op_targets`.

## Layout stability

Pane widths come from `tui/split.rs` `pane_widths(term_cols, fraction)` — never from tree label lengths. Long tree paths, graph subjects, and diff lines clip to that frozen width (diff cells clip and pad by display columns); `left_col_offset` / `right_col_offset` / `diff_col_offset` pan inside the clip. File diffs may instead soft-wrap (`\` / `diff_wrap`, session-only): wrap uses the same display-column width, continuation rows keep a blank gutter and sign, and pan is a no-op until wrap is off. When a focused list or file-diff row moves, the viewport keeps it near the vertical middle (`list_viewport_start`). Graph scrollbar drag and file-diff horizontal thumb drag are the exceptions: they scroll or pan the viewport without moving the cursor. Mouse horizontal wheel (and Shift+wheel) pans the pane under the pointer without moving the focused row. The workspace tree matches the right pane. Trackpad hscroll is SGR `66`/`67`. The live loop enables mouse through `tui/tty.rs` (click + button-event tracking, rxvt 1015, and SGR; never DECSET 1003, because xterm protocol modes 1000/1002/1003 are exclusive and crossterm 0.28 drops motion-bit wheel). The Unix reader decodes SGR, X10, and 1015 the same way as crossterm 0.28. When a file diff has long lines, that report over the left pane pans the diff; short tree paths still pan the tree when the painted diff fits. Default fraction is `TREE_WIDTH_FRACTION` (0.4). The session keeps a `tree_fraction` (resets on next launch; not persisted) and freezes `{ tree_width, tree_inner_width, diff_width }`, recomputing when terminal columns change or when the user drags the divider. Clamp helpers keep both panes ≥ 20 cols (accounting for padding) when the terminal is wide enough. The in-diff side-by-side RULE uses the same session-only drag model. Hit-testing (`hit_split`) consumes `diff_split_rule_x` only while split mode is actually painted (`≥ NARROW_SXS`), and the graph scrollbar column / horizontal track (`graph_scrollbar_x` / `graph_hscrollbar_y`) from the last paint. Graph and file-diff horizontal thumb drag and track jump reuse `SplitDrag` with the pane and in-diff splitters (one mouse stack: `GraphScrollbar` / `GraphHScrollbar` / `DiffHScrollbar`). Hit-testing prefers graph bars, then the file-diff horizontal bar (`diff_hscrollbar_y` from the last paint), then the pane divider, then the in-diff RULE. The vertical graph bar is recorded only after `graph_scroll` leaves 0; the horizontal graph bar only after `col_offset` leaves 0. File-diff bars use the same origin-hidden rule (`diff_scroll` / `diff_col_offset`). The `?` help overlay shrinks the panes instead of overlapping them. It keeps its wrapped row budget. Panes take the leftover rows (this can be fewer than 3).

A 1-row tab strip sits above the panes (`tui/tabs.rs`, painted in `tui/render.rs`). Workspace is always first. Session compare tabs follow. Click a tab to switch. Click `[x]` on a compare tab to close it. Workspace has no close control.

Compare tabs are not a `DrillView` depth. Left is the committed `base...HEAD` file list. Right is `DiffPane`, including when Workspace `drill` is Files or Diff. Graph nesting stays on the Workspace tab. Workspace park stores that drill plus commit-file folds and trie mode. Each compare tab stores its own folds and trie. Watch probes compare endpoints per tab. A probe is not a change until that tab recorded both SHAs. Tabs that are still loading are skipped. A dirty-only tick does not reload compare. `Refresh` forces a reload. Apply drops a closed tab, a changed source, or a stale generation. A compare-file diff keeps a latest request id per tab. Write and merge completion do not recapture the right pane while a compare tab is active. A late Workspace branch-picker or stash-menu prepare does not open on a compare tab. The compare picker list uses `PrepareComparePicker` / `request_prepare_compare`, independent of `b` / `o` branch-picker gens. A compare-picker apply also requires `compare_picker_pending` to still match that checkout.

The ratatui TUI applies crossterm `Resize` to `Terminal::resize` (the event size, not a later ioctl) and recomputes pane widths, graph gutter cap, help overlay rows, and list viewports from the new area. `run_tui` stays synchronous via a current-thread Tokio runtime. One async loop (`tui/event_loop.rs`) fairly `select!`s terminal input (dedicated thread, `tty::poll_event` / `read_event_origin`, bounded batch), watch/fetch deadlines, `JoinSet` completions, flash / Ctrl-C, and a small presenter (dirty flag, ~16–33ms draw cadence, 120ms flash). On Unix the reader tags raw key bytes as `LegacyByte` and CSI-u as `Protocol`. A protocol same-key Press inside a few milliseconds is a typeless echo. A later typeless `g`, including a fast human `gg` under 80ms, completes `gg`. Overlay modes skip watch/fetch ticks. Graph autoload runs only after list movement. Held nav (`h`/`j`/`k`/`l`) accepts Repeat; the input thread drops queued copies of that key after each move. Every git/process effect is `spawn_blocking`. `E` reads blobs and writes temps on that pool (`UserTag::DiffPrepare`); the loop then spawns the tool. A detached GUI wait/cleanup runs on a side thread so it does not hold a JoinSet slot. Fetch / pull / push of independent checkouts share `env_fetch_concurrency` (default 10) on the per-gitdir remote queue and paint progress as each repo finishes. Mixed kinds paint `Fetching 1 · Pulling 1…` or `Fetching 1/2 · queued 1`. Watch/status streams `discover_checkouts` (focused checkout first) then `process_repo`; a slow checkout does not block applying the others or the focused pane. Paths the new discovery omitted (removed worktree, deleted repo) drop immediately — streamed collect never emits `None` for a vanished checkout. Generation ids drop superseded collect / pane / autoload / commit-files / commit-diff / picker / compare-tab / compare-file-diff results. Esc Diff→Files bumps the commit-diff generation so a late result cannot reopen Diff. A watch tick during a collect latches one rerun. First paint is the tree; the initial pane load (and optional startup fetch) enqueue after that. The loop still accepts nav, pane switch, cancel, resize, and quit. Remotes are not a workspace mutex. `Fetch` / `Pull` / `Push` / `FetchTick` stay Handle and enqueue. Exclusive writes and default-branch are Ignore only while exclusive write or default-branch is busy (`classify_busy_dispatch`). An exclusive write on an occupied gitdir still dispatches. Then `schedule` refuses with breadcrumb `busy`. A free gitdir may write while other repos fetch. Quit drops remotes that have not started. In-flight git is unchanged. An unchanged watch poll (tree signatures **and** checkout `HEAD` / sync note / dirty set) skips that pane reload. The next watch tick is due from the start of the interval. Left-pane movement at every drill depth uses the same `RightPaneRequest` through `tui/effect.rs` (TTY `spawn_blocking`): depth 0 tree → graph or worktree diff; depth 1 graph row → commit files; depth 2 commit-file row → that file's diff. `tui/event_pump.rs` `tty_event_loop_must_not_call_sync_pane_git` fails CI if pumps or loop-thread git return.

## Graph widget

`crates/workspace-status-graph` is a ratatui widget for one git graph window. The TUI loads a `GraphModel` in `graph_load.rs` (`log --exclude=refs/stash --all --topo-order --date-order`, window 300, autoload, extra `stash^1`) and paints `GraphWidget`. Graph `o` (graph list or highlighted repo / worktree) can pass selected local tips instead of `--all` so the window is only those ancestors; `O` restores `--all`. Hidden ignored worktrees stay out of `visible_rows` unless `show_ignored` is true. Worktree marks are linked extras from `git worktree list` (`.git` gitfile); the main checkout is not marked. Gutter cells use `GraphCell.color_lane` and the active theme's eight lane colours (`T` cycles; `GraphWidget::lane_colors`. An empty slice still falls back to `DEFAULT_LANE_COLORS`). Chrome is a 2-line selection footer (`selection_detail_lines` / `selection_detail_parts`) plus optional sync header (`graph_chrome_budget`). `M` expands that footer (`selection_footer_parts`) to wrap git `%s` plus `%b` (body capped at 8 KiB). Graph list rows stay one line. The commit-files header uses the same expand flag. Footer ref chips reuse the same `LabelKind` runs as the commit spacer so `GraphWidget::label_palette` paints HEAD / default / feature / remote / tag with the row-chip colours (not a single footer wash). A loaded graph always emits the working-tree row. Commit and stash spacers reuse the same hash / date / author drop order. Dates are relative through 3 hours, then local `YYYY-MM-DD HH:MM`. The gutter is capped (`gutter.rs`: at most 30% of the pane, keep ≥24 columns for refs+subject) with one left-aligned clip for every row so rails stay column-aligned when subjects clip. Overflow branch/tag chips: truncate the next name with `…` (keep brackets) when part of it fits; a bold `[+N]` on the row itself counts only fully hidden chips (heading accent, not muted meta; omit `[+N]` when the last painted chip is merely truncated). The footer lists every full ref (HEAD commit refs on the working-tree row).
...
| File-diff syntax highlighting | `tui/syntax.rs` (path/extension detect, syntect via `two-face`, viewport + hunk/section streams) + `paint_cell_spans` in `tui/render.rs`. Compare tabs use the active compare file path. Span cache keys on a hash of the painted diff text, not the `DiffContent` address. Add/del row backgrounds are `palette.diffAddBg` / `diffDelBg` in `tui/theme.rs`. Intra-line word diff stays out of scope. |
...
| Compare tabs / picker | `tui/tabs.rs` + `CommitFileSource::Compare` + `tui/state/` + `tui/effect.rs` (`LoadCompare*` / `PrepareComparePicker`) + `scheduler.rs` `request_prepare_compare` + palette commands + `render.rs` tab strip. No `NodeKind`. No graph crate change. |
| New pane / overlay    | `tui/` module (`command_palette.rs` for `Ctrl-k` / `:`) + `chrome.rs` row budget + `render.rs` layout                                                                                                               |
| TUI comments / markdown export | `tui/comments/` (`store`, `target` / GC, overlay, markdown export, entity reference) + `tui/persist.rs` (lock + merge + atomic write) + `;` / `V` / `y` / `'` / overlay `Ctrl-R` in `tui/keys.rs` + `help.rs` + `chrome.rs` overlay budget + `render.rs`. The comment overlay is a multiline caret editor (`CommentPrompt` wraps `tui-textarea`; Shift+Enter newline; Ctrl-A/E line; Ctrl-Left/Right word). It replaces the idle status row. Overlay Ctrl-R toggles `CommentPrompt.resolved`. Enter persists body and resolve state (`CommentEntry.resolved` in `comments.json`). A failed persist sets `comment save failed` plus a short IO error on the breadcrumb trailing slot (never `comment saved` / `comment deleted`). Version 2 namespaces records under `workspaces.<id>` (SHA-256 of canonical cwd). GC and save replace only the current workspace bucket. A version-1 file keeps its records under `__legacy__`. Load uses that map when the current workspace bucket is missing. GC of one cwd does not drop `__legacy__`. A missing file starts empty. A present file that is not valid v1/v2 UTF-8 JSON is left unchanged and persist returns an error. `V` is visual-line highlight on a focused file diff. Without highlight, `;` opens the covering stored span. Highlight drops when the painted row list changes. `y` copies the focused tree / graph / commit-file / diff row and descendants under that row (after GC). Resolved comments stay in that copy and carry `[resolved]`. A line comment with a newline stays one markdown bullet. Continuation lines are quoted. `'` copies a pasteable entity reference for the focused row (`Effect::CopyClipboard` with `announce: true`). Status is `copied` when the copy succeeds. Status is `copy failed` when OSC 52 and the host tool both fail. No overlay. No comment store write. GC uses snapshot `local_branches` (snapshot worker), not git on apply. Status-failed snapshots keep last-good branch names only when every checkout of that identity has an empty counted list. |
...
| New key binding       | `tui/keys.rs` (`Action`) + `tui/state/dispatch.rs` + `dispatch_keymap.rs` / `pan.rs` / `dispatch_drill.rs` / `dispatch_write.rs` + `tui/help.rs` (`HELP_GROUPS`) + `tui/command_palette.rs` for named-command catalog + `tui/gates.rs` if the key is row-scoped |
...
| Mouse enable / mouse decode / key origin | `tui/tty.rs` (live `poll_event` / `read_event_origin` / `enable_mouse`; Unix raw-byte vs CSI-u tag; Unix SGR / X10 / 1015 decode; lone ESC waits one non-zero poll timeout; hangup / 0-byte read is an error; SGR decode through `decode_sgr_mouse`) + the input thread in `tui/event_loop.rs` |
| Shared TUI e2e seed / hscroll oracle | `crates/workspace-status/tests/common/` — used by `tui_tty_e2e`. Not a second harness. See [tui-tty-e2e.md](./tui-tty-e2e.md) |
| Real-TTY TUI e2e | `crates/workspace-status/tests/tui_tty_e2e/` (PTY in `cargo test`; xfce keys + xterm XTEST wheel in Actions `tui-tty-desktop`). See [tui-tty-e2e.md](./tui-tty-e2e.md) |
...
**Real-TTY TUI e2e** spawns the `workspace-status` binary on a PTY and writes keys / xterm SGR mouse bytes so `event::read` is the live loop. Desktop (xfce4-terminal keys; xterm XTEST wheel) runs in GitHub Actions `tui-tty-desktop` because hosted runners have no trackpad. That job, and `scripts/capture-demo-stills.sh`, start Xvfb / dbus / Openbox through `scripts/with-desktop-session.sh`. See [tui-tty-e2e.md](./tui-tty-e2e.md).

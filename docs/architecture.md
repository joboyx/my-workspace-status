...
| TUI comments / markdown export | `tui/comments/` (`store`, `target` / GC, overlay, markdown export) + `;` / `y` in `tui/keys.rs` + `help.rs` + `chrome.rs` overlay budget + `render.rs`. GC uses snapshot `local_branches` (snapshot worker), not git on apply. Status-failed snapshots keep last-good branch names (or skip branch wipe when the list is empty). |

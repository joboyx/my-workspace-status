...
| Mouse enable / mouse decode / key origin | `tui/tty.rs` (live `poll_event` / `read_event_origin` / `enable_mouse`; Unix raw-byte vs CSI-u tag; Unix SGR / X10 / 1015 decode; lone ESC waits one non-zero poll timeout; hangup / 0-byte read is an error; Headless SGR through `decode_sgr_mouse`) + the input thread in `tui/event_loop.rs` |

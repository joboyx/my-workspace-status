...
| File-diff syntax highlighting | `tui/syntax.rs` (path/extension detect, syntect via `two-face`) + `paint_cell_spans` in `tui/render.rs`. Add/del row backgrounds are `palette.diffAddBg` / `diffDelBg` in `tui/theme.rs`. Intra-line word diff stays out of scope. |

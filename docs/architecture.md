...
Compare tabs are not a `DrillView` depth. Left is the committed `base...HEAD` file list. Right is `DiffPane`. Graph nesting stays on the Workspace tab. Watch probes compare endpoints per tab. A probe is not a change until that tab recorded both SHAs. Tabs that are still loading are skipped. A dirty-only tick does not reload compare. `Refresh` forces a reload. Apply drops a closed tab, a changed source, or a stale generation. A compare-picker apply also requires `compare_picker_pending` to still match that checkout.
...
| Compare tabs / picker | `tui/tabs.rs` + `CommitFileSource::Compare` + `tui/state/` + `tui/effect.rs` (`LoadCompare*`) + palette commands + `render.rs` tab strip. No `NodeKind`. No graph crate change. |

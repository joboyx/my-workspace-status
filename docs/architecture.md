...
| TUI comments / markdown export | `tui/comments/` (`store`, `target` / GC, overlay, markdown export) + `;` / `V` / `y` in `tui/keys.rs` + `help.rs` + `chrome.rs` overlay budget + `render.rs`. The comment overlay is a caret editor (`CommentPrompt.cursor`); it replaces the idle status row. `V` is visual-line highlight on a focused file diff. Without highlight, `;` opens the covering stored span. Highlight drops when the painted row list changes. `y` copies the focused tree / graph / commit-file / diff row and descendants under that row (after GC). GC uses snapshot `local_branches` (snapshot worker), not git on apply. Status-failed snapshots keep last-good branch names only when every checkout of that identity has an empty counted list. |

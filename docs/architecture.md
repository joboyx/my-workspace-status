...
| Mouse enable / SGR decode / key origin | `tui/tty.rs` (live `poll_event` / `read_event_origin` / `enable_mouse`; Unix raw-byte vs CSI-u tag; Headless SGR through `decode_sgr_mouse`) + the input thread in `tui/event_loop.rs` |

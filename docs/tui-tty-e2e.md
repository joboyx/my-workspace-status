...
Not claimed on a real TTY (TestBackend and/or no safe operator oracle yet): `n`/`N` pane next/prev, Home/End, PgUp/PgDn, Ctrl-u/d, `e` editor (spawns vim), `T` theme (colour only), `m` mouse toggle, double-click Enter, vertical wheel, divider/scrollbar drag, second Ctrl+C quit, `Y` revert+delete, graph `b` checkout, `d` skip-when-dirty. Worktree **add** is not a TUI key (`W` is remove only).

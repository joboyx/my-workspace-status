...
- Claims kept because they fail on a no-op: launch paint (wait for `+dirty`, not only the tree — right-pane git is pumped), `?` help, graph drill, graph branch focus `o`/`O`, Shift+letter CSI-u (`O` clear focus, `S` stash, `/` capitals), graph-focus unmark-then-Enter, Ctrl+C quit prompt, PTY SGR tree pan, desktop help/search, desktop xfce Shift keys (space toggles `[x]`; unmark-then-Enter before `O`), desktop xterm tree pan. Stage / reviewed / fold / diff-pan / click are not claimed here.

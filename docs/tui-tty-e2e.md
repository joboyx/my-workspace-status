...
- Desktop xfce `/` arm: wait for SEARCH and the live jump (graph subject) before Return. xfce can drop Enter while right-pane git runs. PTY byte writes do not have that race. Graph stash pop on xfce uses `j` / Tab / `j` / `p`, not SEARCH.

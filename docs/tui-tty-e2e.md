...
- Claims kept because they fail on a no-op: launch paint, `?` help, graph drill, graph branch focus `o`/`O`, Shift+letter CSI-u (`O` clear focus, `S` stash, `/` capitals), graph-focus unmark-then-Enter, Ctrl+C quit prompt, PTY SGR tree pan, desktop help/search, desktop xfce Shift keys, desktop xterm tree pan. Stage / reviewed / fold / diff-pan / click are not claimed here.

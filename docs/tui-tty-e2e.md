...
| `m` mouse toggle | `pty_m_toggles_mouse_capture` | — | Tree `m` (raw byte). Default-on click selects. `Mouse off` ignores SGR click. `Mouse on` selects again. Not graph merge |

...
| Stash `S`/`a`/`D` | `pty_stash_create_apply_and_drop` | `desktop_xfce_stash_create_apply_and_drop` | After first paint, CSI-u Shift+S opens the create-only overlay on the dirty README. Overlay `s` stashes (`Stashed 1 file`); README leaves the tree. Graph `a` applies (`applied stash@{0}`); README returns; stash stays. CSI-u Shift+D then `y` drops; README stays dirty. A no-op, stage `S `, Space `*`, overlay-open-only, pop, or merger stash cannot pass. Not graph `p` pop. Desktop twin unchanged. |
| Graph stash `p` pop | `pty_stash_graph_pop` | `desktop_xfce_stash_graph_pop` | After first paint, `j` onto merger and Tab onto the graph. `j` selects `stash@{0}`. Graph `p` pops that stash (apply + drop): toast `popped stash@{0}`, `wip.txt` returns on the merger tree (`A`), `stash@{0}` leaves the graph. Not workspace `p` pull. Not stash `S`/`a`/`D`. A no-op, apply-only, drop-only, or toast-only cannot pass. Desktop waits for SEARCH + graph jump before Return. |
...
| `z` fold | `pty_z_folds_focused_repo` | — | After first paint, `z` on the dirty file is a no-op (file stays; not Space `*`; not stage). `k` focuses expanded `app`. `z` folds that repo (`>`, README gone, cursor stays). Graph-focused `z` is a no-op. A late `z` toggles the row open. Not `h`/`l`. Not `zz` subtree. A no-op, `/` hide, or workspace-root fold cannot pass. |

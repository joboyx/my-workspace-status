...
- PTY leftover can decode OSC 52 clipboard payloads from the master byte stream (`PtySession::clipboard_payloads` / `last_clipboard`). `vt100` strips OSC 52 from the painted screen, so copy claims must read the raw bytes (`ESC ] 52 ; c ; <base64> BEL` or ST). Adding a `pty_*` test still does not require other edits to this file.
...
- Desktop open waits for idle `? help` chrome (not a space-padded pane title). xdotool key/type uses `--clearmodifiers` so a prior Shift chord cannot turn `/` into help `?`.
...
- Typeless CSI-u (VTE that omits event types) is `CSI code ; 1 u`. Human `gg` on that path is two of those reports with a gap above the few-millisecond protocol echo window (`G_CHORD_PROTOCOL_ECHO_MS` in `tui/keys.rs`). A 40ms gap is inside the 400ms double-tap window and must complete `gg`. After the completing `g`, wait out that burst so the next `g` of `gt` is not dropped. Do not inject a CSI-u Release or a second typeless report for key-up when the test claims that path. A raw-byte `gg` helper stays two `g` bytes and also must not inject Release. `pty_live_g_chords` drives this encoding. Immediate typeless press+echo `gt` stays in leftover `pty_compare_gt_chords_and_gg`.

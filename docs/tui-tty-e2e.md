...
GitHub-hosted runners have no physical trackpad. The PTY path writes the same SGR bytes a terminal sends for trackpad hscroll (`CSI < 67` wheel right, `CSI < 69` Shift+wheel down) into a live `event::read` loop. The desktop path asks VTE to encode wheel (xdotool button 7) so a DECSET/mouse-mode regression still fails.
...
xvfb-run -a -s "-screen 0 1600x1000x24" bash -c '
  ...
  openbox >/tmp/ws-tty-e2e-openbox.log 2>&1 &
  sleep 0.4

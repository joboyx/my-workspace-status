...
| Graph `o` / `O` focus | `pty_graph_branch_focus_overlay` | `desktop_xfce_shift_keys_search_and_clear_focus` | `o` overlay on the current `*` branch. Enter applies. CSI-u `O` restores `--all`. Toast + clear-focus hint. Not files drill. Not `/`. |

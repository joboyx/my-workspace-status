...
- Typeless CSI-u (VTE that omits event types) is `CSI code ; 1 u`. Human `gg` on that path is two of those reports with a gap above the ~80ms protocol echo window. Do not inject a CSI-u Release or a second typeless report for key-up when the test claims that path. A raw-byte `gg` helper stays two `g` bytes and also must not inject Release.

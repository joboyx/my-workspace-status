...
Long diff lines are **not** word-wrapped. `h` / `←` and `l` / `→` pan when the right pane shows a file diff (same keys pan a focused graph or commit-file list). Shift+Left / Shift+Right pan the focused pane, including the tree. Offset resets to 0 when the focused file changes. Header shows `· pan N` when offset > 0. Rows stay clipped to the pane width.

...
Fetch / pull / push / default-branch / watch snapshot / full reload / focused-repo reload / local-write snapshot collect / commit-file list and commit diff / stash-menu and branch-picker git run on a worker thread (`run_work_pumped`). The draw loop stays live: resize still relayouts, `q` still leaves after the current git child exits, and other keys are drained so the TTY buffer cannot fill. Git itself is started with stdin `/dev/null` and `GIT_TERMINAL_PROMPT=0` so a credential prompt cannot deadlock the TUI. Applying those results also reloads the right pane (`git log` / file `diff`) on the same worker — that follow-up used to run on the draw thread and could queue keys until they flushed in a burst. An unchanged watch poll skips the pane reload. Graph autoload (`loading older…`) uses the same pump when a TTY is mounted. Sync pane git (`load_right_headless`) is Headless e2e only; `tty_event_loop_must_not_call_sync_pane_git` fails CI if TTY grows a `load_right(` / unpumped `reload_snapshot(state` again. Search, help, and mouse drag stay in-memory. A TTY editor unmounts the alternate screen (intentional); after it returns, repo snapshot + pane git are pumped. Local `git add` / stash / checkout / merge children stay on the event thread (typically short); the follow-up workspace collect is pumped. `cargo test` includes `snapshot_contract.rs` and `tui_daily_e2e.rs`.

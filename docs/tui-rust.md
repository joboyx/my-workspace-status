...
Graph history matches Ink `gitLogGraphWindow`: `log --exclude=refs/stash --all --topo-order --date-order --skip --max-count` with a 300-commit window. Reaching the last loaded row fetches the next page and paints `loading older…`. Missing `stash^1` parents are loaded with `log --no-walk --ignore-missing` and sit after the log prefix so autoload skip stays on the window, not `commits.len()`. The working-tree row is always present on a loaded graph. The pane keeps a 2-line selection footer (footer before header when space is tight). Each gutter cell uses its `color_lane` from `DEFAULT_LANE_COLORS`.

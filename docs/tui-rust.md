...
Fetch / pull / push / default-branch / watch snapshot / full reload run on a worker thread. The draw loop stays live: resize still relayouts, `q` still leaves after the current git child exits, and other keys are drained so the TTY buffer cannot fill. Git itself is started with stdin `/dev/null` and `GIT_TERMINAL_PROMPT=0` so a credential prompt cannot deadlock the TUI. Applying those results also reloads the right pane (`git log` / file `diff`) on the same worker — that follow-up used to run on the draw thread and could queue keys until they flushed in a burst. An unchanged watch poll skips the pane reload. Graph autoload (`loading older…`) uses the same pump when a TTY is mounted.

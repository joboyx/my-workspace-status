...
Fetch / pull / push / default-branch / watch snapshot / full reload run on a worker thread. The draw loop stays live: resize still relayouts, `q` still leaves after the current git child exits, and other keys are drained so the TTY buffer cannot fill. Git itself is started with stdin `/dev/null` and `GIT_TERMINAL_PROMPT=0` so a credential prompt cannot deadlock the TUI.

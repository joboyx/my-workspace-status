...
| `S` | Stash menu. Left-pane only (right-focused graph is a silent no-op). Tree dirty file or repo is create-only (`s`). Graph commit or uncommitted (depth 1 left / graph list) offers apply / pop of the latest stash; drop needs a focused stash row (`a` / `p` / `D`). Hidden / no-op at depth ≥ 2 |
| `Enter` | Left: focus right at the same depth. Right: push the next ViewStack depth and stay on the right (graph → commit files → diff). No-op at depth 2 |
| `Esc` | Right: unfocus to the left at the same depth. Left: pop one depth and stay on the left (no-op at depth 0). Never quits |
...
- File diffs paint `{repo}/{path}  inline|split` (plus ` · full` / ` · pan N`) then STAGED / UNSTAGED / NEW labels and a line-number gutter. At depth 2 the left pane is the commit file list, so that header is still how the path stays visible on the diff. Intra-line / syntax highlight is not implemented yet
...
- `S` on a dirty tree file or repo is create-only (`s stash`). `S` on a clean tree row that has `stash@{0}` is a no-op. `S` is left-pane only: a right-focused graph is a silent no-op. Hidden / no-op at depth ≥ 2. On a left-focused graph (depth 1) or when the menu is already open, a graph commit or uncommitted row offers apply / pop of the latest stash (`a` / `p`); drop stays off unless a graph stash row is focused. Graph stash rows offer apply / pop / drop of that `stash@{n}` (`a` / `p` / `D`, plus `S` on a left-focused graph stash). Never drop latest from a non-stash row. Menu `p` runs immediately. Drop asks `y` / `n` in a boxed overlay
...
- `Enter` on a graph commit (or stash / uncommitted row) opens depth-1: left pane is the graph list, right pane is commit detail (short hash, refs, subject, author, relative date, plus the file tree). `Enter` on a file opens depth-2: left pane is the commit file list, right pane is the commit diff. Left `Enter` always focuses right first. Right `Enter` pushes the next depth and stays on the right (no-op at depth 2). `Esc` on the right unfocuses to the left; `Esc` on the left pops one depth and stays on the left (no-op at depth 0). Esc never quits. Hidden ignored repos stay out of the drill unless shown
...
- Linked worktrees under a family are checkout rows labeled by **branch**, not `wt <path>`. Detached linked checkouts fall back to the short worktree path. Linked-only snapshots (no primary in the window) stay a flat `Repo` row — no phantom primary container.
...
Left pane: workspace tree at depth 0. Clean default-branch repos sit under a folded `No updates` group. Depth 1 left is the graph list. Depth 2 left is the commit file list.
...
A file diff paints a one-line path header (`{repo}/{path}  inline|split`, plus ` · full` when unlimited `-U` is on, ` · pan N` when panned) then STAGED / UNSTAGED / NEW section labels and a line-number gutter. At depth 2 the left pane is the commit file list, so that header is how the file path stays visible on the diff. Intra-line and syntax highlight are not implemented yet. Destructive confirms are boxed overlays.
...
Crossterm `Resize` is a first-class action. The event loop calls `Terminal::resize` with the event size (ioctl can still report the previous size). Pane widths, graph gutter cap, help overlay height (wrap math from terminal columns), and list viewports recompute from the new area. `q`, Tab, Home/End, family-row `b`, and picker `C` stay.

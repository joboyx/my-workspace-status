...
Reviewed marks use `$XDG_STATE_HOME/my-workspace-status/viewed-files.json` (same identity and fingerprint as Ink). A mark drops when the file fingerprint changes. Space toggles dirty workspace-tree file rows at depth 0 only. The viewed glyph is Ink `ICON_VIEWED` (`` / ASCII `*`), cyan/blue, trailing before the status badge — not the clean check. Clean `ICON_CLEAN` (`` / `.`) paints only on the No updates group and on repo / checkout rows inside it.

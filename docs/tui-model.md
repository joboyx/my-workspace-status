...
| `Checkout`  | `checkout:<repoPath>`        | dirs/files                  | Nested under a family container only. Primary uses `ICON_BRANCH` + branch (never `ICON_LINKED_WORKTREE`, never the merge mark). Linked extras use `ICON_LINKED_WORKTREE` + branch (or short path when detached) + merge mark. Files live here, not on the container. |

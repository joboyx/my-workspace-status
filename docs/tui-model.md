...
Pure data under `crates/workspace-status/src/tui/` (tree, fold, flatten, keymap, live refresh). The event loop is `Action` → `AppState::dispatch` → `Effect`. `tui/effect.rs` turns that `Effect` into Scheduler jobs and applies results. The live TTY loop (`tui/event_loop.rs`) spawns those jobs on a `JoinSet`. Headless e2e calls `Interpreter::interpret_sync` (same schedule and apply; jobs run on the test thread).
...
| `Checkout`  | `checkout:<repoPath>`        | dirs/files                  | Nested under a family container only. Primary uses `ICON_BRANCH` + branch (never `ICON_LINKED_WORKTREE`, never the merge mark). Linked extras use `ICON_LINKED_WORKTREE` + branch (or short path when detached) + merge mark. Checkmark only when HEAD is a strict ancestor of the default tip; HEAD equal to that tip is open. Files live here, not on the container. |
...
| `Dir`       | `dir:<repoPath>:<fullPath>`  | dirs/files                  | Tree mode only. Single-child chains are collapsed (`apps/web/packages/ui` as one node).                                                                                                                                                                            |
...
`list_focus_target` (`tui/state/`) is the focused list: depth 0 left → tree; depth 0 right / depth 1 left → graph; depth 1 right / depth 2 left → commit files; depth 2 right (and depth 0 file diffs) → none (file-diff focused row). `j`/`k`, `gg` / `G` / Home / End, fold, search, click, and vertical wheel follow that target. The viewport keeps the focused row near the vertical middle (same rule as the workspace tree). On a focused file diff, `gg` / `G` (and Home / End) move to the first / last row. CSI-u key release does not move and does not clear the `gg` pending (`GPending`). Mouse horizontal wheel (and Shift+wheel) pans the pane under the pointer without moving the focused row. The workspace tree matches the right pane. Trackpad hscroll is SGR `66`/`67`. When a file diff has long lines, that report over the left pane pans the diff. Short tree paths still pan the tree when the painted diff fits. Click still selects. Unshifted `h` / `l` still fold the tree.
...
Enter that deepens the stack keeps **right** focus. Esc cancels visual-line highlight (`V`) first. Esc that pops a depth keeps **left** focus. Esc never quits.
...
`AppState` in `tui/state/` holds restorable view state: cursor row id, folds, filter, diff mode, full-context ids, tree mode, mouse enabled, theme, nav (focus pane + drill depth), `diff_cursor`, `diff_col_offset`, `left_col_offset`, `right_col_offset`, and session-only `diff_visual_anchor` (`V` visual-line highlight on a focused file diff). The file-diff viewport (`diff_cursor` / `diff_scroll` / `diff_col_offset`) belongs to one view identity: a workspace `repo`+`path`, or a commit `repo`+source+`path`. A new identity resets those fields to the origin and clears highlight. A same-view reload keeps the viewport. Highlight also drops when the painted row list changes (watch reload, split/inline, resize). Graph window default is 300. Graph branch focus (`o` / `O`) is session-only (`graph_branch_focus`): a per-repo list of local names whose ancestors the graph shows, or `None` for `--all`. Fold, split ratios, pan offsets, theme cycle, and graph focus reset on the next launch. Reviewed marks persist in `viewed-files.json`. Line and object comments persist in `comments.json` (`tui/comments/`, `WS_STATUS_COMMENT_STORE` overrides). Version 2 namespaces both files by workspace identity (SHA-256 hex of the canonical cwd). GC and save replace only the current workspace bucket. Refresh GC uses snapshot `local_branches` and checkout paths (no git on apply or `;`). A primary detached HEAD is a worktree path key, not `HEAD (detached)` as a branch. Commit comments stay while that repo identity is still in the snapshot. Branch and working-tree line comments stay when `git status` fails (`status failed`, empty `local_branches`) only if every checkout of that identity has an empty counted list. A successful sibling's live names win. The GitHub Release startup prompt (`update_check.rs`) runs in the CLI **before** `AppState` exists; it is not session state.
...
Graph rows come from `workspace-status-graph` `GraphModel::visible_rows`. Graph `o` can restrict the loaded window to ancestors of selected local branches (`git log <branches>` instead of `--all`); overlay Enter applies visible marks or the cursor row (hidden pre-marks do not leak through a filter). Unmarking every `[x]` then Enter, or `O`, restores `--all`. Stashes park immediately above `stash^1`. Pane `/` matches a graph row on subject, author, painted relative date or local `YYYY-MM-DD HH:MM`, UTC `YYYY-MM-DD HH:MM`, branch and tag names, and full or short sha (stash / worktree rows use those fields when present). Graph pane chrome: 2-line selection footer when height ≥ 3 (`graph_chrome_budget` / `selection_detail_lines` / `selection_detail_parts`; footer preferred, header drops first). Uncommitted: `Working tree clean` / `Uncommitted changes` + HEAD commit ref chips, or `worktree · not a commit` when HEAD has none. Spacer: `…` + `connector · not selectable`. Stash: subject + `stash@{n} ·` hash `·` date. Commit: subject + ref chips `·` hash `·` author `·` date, or `(no refs)`. Empty: `no selection`. Footer chips use the same `LabelKind` palette as the row (HEAD / default / feature / remote / tag), not a flat wash. Graph PageUp/PageDown move in painted list space (`visible − 1` painted lines, snap to selectable); `j`/`k`, click, and vertical wheel stay on selectable `visible_rows`. The viewport keeps the focused row near the vertical middle. The 1-column scrollbar thumb is painted only after the list leaves the top; the horizontal bar only after the viewport leaves the left edge. Both are mouse-draggable through the same `hit_split` / `SplitDrag` stack as the pane and in-diff splitters (`SplitDrag::GraphScrollbar` / `GraphHScrollbar`); a track click jumps toward that position without moving the graph cursor. Horizontal mouse wheel (and Shift+wheel) pans the pane under the cursor without changing keyboard focus (SGR `66`/`67`).

See [git-graph-topology.md](./git-graph-topology.md) and [graph.md](./graph.md).

## Actions and gates

`tui/action.rs` is the single source of truth for which actions exist. `tui/tty.rs` enables mouse (click + button-event + SGR, never DECSET 1003) and decodes TTY SGR reports the way the live `event::read` loop does (`66`/`67` → `ScrollWheel { horizontal: true }`; motion-bit `98`/`99` dropped). `tui/keys.rs` maps crossterm events to `Action`. Keyboard enhancement reports Shift+letter as the unshifted codepoint plus SHIFT; the keymap folds that to the uppercase binding (`O` / `S` / `/` capitals) so it matches a raw `'O'` byte. `tui/state/dispatch.rs` routes `Action` to session updates and `Effect`. List keys live in `tui/state/dispatch_keymap.rs`. Pan / wheel live in `tui/state/pan.rs`. Enter / Esc live in `tui/state/dispatch_drill.rs`. Git writes live in `tui/state/dispatch_write.rs`. `mouse_pan` routes a long file-diff pan when the pointer is over the left pane and the painted diff can pan; otherwise the left list pans. `tui/gates.rs` hides writes that the focused row / depth / pane must not run. `tui/ops.rs` resolves stage / unstage / revert file lists (`collect_write_files`) and bulk git targets (`op_targets` / `push_targets`).

`;` opens a boxed comment overlay on a numbered file-diff line (working-tree or commit) or on a commit / non-default branch / worktree row. The overlay is a multiline caret editor (`tui-textarea`): Shift+Enter inserts a newline; Enter saves; empty Enter deletes. Left / Right / Home / End move the caret (Home / End are the current line). Ctrl-A / Ctrl-E are line start / end. Ctrl-Left / Ctrl-Right move by word. Insert, Backspace, and Delete apply at the caret. Overlay Ctrl-R toggles resolved. Enter persists body and resolve state. The idle status row is replaced. Hint chips do not paint inside the box. `V` on a focused file diff starts visual-line highlight; `j` / `k` and arrows extend the range; `;` then attaches the comment to that line span. Without highlight, `;` opens the stored span that covers the focused line (smallest inclusive span, then lowest start line, then key order). Empty Enter deletes that span. Esc leaves highlight without commenting. Watch reload, split versus inline, or a painted-row change drops highlight. The diff gutter reserves a comment-mark column on every numbered cell so the line numbers do not shift when a comment appears. Open comments paint `ICON_COMMENT` (`"` / nf-fa-comment). Resolved comments paint `ICON_COMMENT_RESOLVED` (`'` / nf-fa-comment-o). Workspace, group, dir, file, and default-branch rows are not object-comment targets. If a repo has exactly one non-default branch, a comment that would have attached at repo scope attaches to that branch. A primary detached HEAD is a worktree path key. Graph rows paint the comment glyph when the row has an object comment or a file-line comment (resolved glyph when every comment on the row is resolved). Commit-file rows paint the same trailing mark when that file has a line comment. `y` copies live comments as markdown after GC for the **focused** tree / graph / commit-file / diff row and descendants under that row (a file copies that path only; a folder copies files under it). Unrelated siblings stay out. The workspace row copies every live comment. Resolved comments stay in that copy and carry `[resolved]`. A line comment with a newline stays one markdown bullet. Continuation lines are quoted. Confirm-mode `y` is still revert-yes.

...
Helpers in the same module: `graphWritesAllowed` (target is `graph`), `fileWritesOnDiffAllowed` (`edit` / `fullFile` / `toggleViewed` on a focused diff), `foldAllowed` (tree or commit-files only), `easyMotionListTarget` / `easyMotionPaintSlot`, `fullContextToggleId` (same id `ctrl+o` toggles and Esc clears). Graph pane writes in `runAction` (`graphCheckout` / `graphCreateBranch` / `stashApply` / `stashDrop` / `stashPop`) gate on `isGraphListFocused({ depth, focusPane, graphVisible })` from `graph/focus.ts` (depth 0 right, depth 1 left, or any later depth where the focused pane is the graph list).
...
The `Action` union covers move / `moveTo`, fold/expand/collapse, toggleTreeMode / toggleCommitTreeMode / toggleShowIgnored, cycleTheme, stage/unstage/revert, toggleDiffMode, refresh, edit, `toggleViewed`, `fullFile`, `searchStart` / `searchNext` / `searchPrev`, `easyMotionStart`, `pageMove`, help, quit, confirmYes / confirmYesClean / confirmNo, scrollDiff, `panDiff`, toggleMouse, `navEnter` / `navEsc`, `graphCheckout` / `graphCreateBranch` / `stashApply` / `stashDrop` / `stashMenu` / `stashPop`, none. `edit` records an `EditRequest` via `onEditRequest` and returns `'quit'` so the mount ends in the same keypress; `run.ts` then opens `$EDITOR` and remounts. It does **not** write `ExitReason.type === 'edit'` from the action.

`stage` / `unstage` / `revert` resolve targets through `collectFiles` in `scope.ts` (file / dir / checkout / flat repo). Family containers yield no files — write on a checkout child. Revert opens a counted confirm (`PendingConfirm` with `kind: 'revert'`); `y` discards tracked only, `Y` also deletes untracked via per-file `removeUntrackedFile`.
...
`stashMenu` / `stashPop` are handled in `useAppState` (not `USE_ACTIONS_FORWARDED`). Overlay `p`/`d` do not steal registry `p` pull / `d` default-branch while `stashMenuMode` is set.

Pure gates live in `graph/actions.ts`. Overlays replace StatusBar while open (same pattern as `BranchPicker`). Depth 0 **tree** `b` stays ActionId `branch`; depth 0 **graph** `b` is `graphCheckout`.
...
- Row text (layout A): commit row 1 = gutter + subject; stash row 1 = leaf-tip gutter + subject (no diamond in subject text); spacer row 2 = rails (commits) or live-gap rails + **short leaf spur** toward `stash^1` when parked (orphans: through-rails only) + chips/`stash@{n}` left + right-anchored hash / date / author. Pane width drives the flex budget (not a hardcoded 80); gutter uses a hybrid cap (`≤30%` of list width and leave ≥24 cols for subject) with commit-lane-anchored clipping when topology exceeds the cap (`gutterBudget.ts`). Focus paints the full 2-row pair.
- GraphPane chrome: 2-line selection footer (full subject + refs/hash/author/date) when height ≥ 3; 1-line sync header (`branch` + `tuiSyncMark`) when remaining space allows — footer preferred, header drops first / omitted when no sync snap (`graphChromeBudget` / `graphSelectionDetailLines`).

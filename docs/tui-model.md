...
- GraphPane chrome: 2-line selection footer when height ≥ 3 (`graphChromeBudget` / `graphSelectionDetailLines`; footer preferred, header drops first / omitted when no sync snap). Uncommitted: `Working tree clean` / `Uncommitted changes` + `worktree · not a commit`. Spacer: `…` + `connector · not selectable`. Stash: subject + `stash@{n} ·` hash `·` date. Commit: subject + ref chips `·` hash `·` author `·` date, or `(no refs)`. Empty: `no selection`. 1-line sync header (`branch` + `tuiSyncMark`) when remaining space allows.

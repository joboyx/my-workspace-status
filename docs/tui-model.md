...
`;` opens a boxed comment overlay on a numbered file-diff line (working-tree or commit) or on a commit / non-default branch / worktree row. Empty Enter deletes. Workspace, group, dir, file, and default-branch rows are not object-comment targets. If a repo has exactly one non-default branch, a comment that would have attached at repo scope attaches to that branch. `y` copies live comments as markdown after GC and shows them in an overlay. Confirm-mode `y` is still revert-yes.

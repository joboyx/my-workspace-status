...
`AppState` in `tui/state/` holds restorable view state: cursor row id, folds, filter, diff mode, full-context ids, tree mode, mouse enabled, theme, nav (focus pane + drill depth), `diff_col_offset`, `left_col_offset`, and `right_col_offset`. Graph window default is 300. Graph branch focus (`o` / `O`) is session-only (`graph_branch_focus`): a per-repo list of local names whose ancestors the graph shows, or `None` for `--all`. There is no on-disk session store — fold, split ratios, pan offsets, theme cycle, and graph focus reset on the next launch. The GitHub Release startup prompt (`update_check.rs`) runs in the CLI **before** `AppState` exists; it is not session state.

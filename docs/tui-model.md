...
- `watch.rs` attributes file flashes with file node ids and op-row flashes with repo node ids. Chrome flashes walk the tree and skip file nodes — files stay on status+mtime signatures. Graph and commit-file rows use separate identity maps (`{repo}#{kind}:{id}` and `{repo}#{source}#{row_id}`) so a repo switch is a new list, not a mass flash. Add/update/remove flash the same row only. Removals keep a ghost row for the 800ms fade. The flash is a background fade (four-step ramp); status letter colours stay.

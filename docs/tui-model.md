...
`;` opens a boxed comment overlay on a numbered file-diff line (working-tree or commit) or on a commit / non-default branch / worktree row. The overlay is a one-line caret editor: Left / Right / Home / End move the caret; insert, Backspace, and Delete apply at the caret. The idle status row is replaced. Hint chips do not paint inside the box. `V` on a focused file diff starts visual-line highlight; `j` / `k` and arrows extend the range; `;` then attaches the comment to that line span. Without highlight, `;` opens the stored span that covers the focused line (smallest inclusive span, then lowest start line, then key order). Empty Enter deletes that span. Esc leaves highlight without commenting. Watch reload, split versus inline, or a painted-row change drops highlight. The diff gutter reserves a comment-mark column on every numbered cell so the line numbers do not shift when a comment appears. Workspace, group, dir, file, and default-branch rows are not object-comment targets. If a repo has exactly one non-default branch, a comment that would have attached at repo scope attaches to that branch. A primary detached HEAD is a worktree path key. Graph rows paint `ICON_COMMENT` (`"` / nf-fa-comment) when the row has an object comment or a file-line comment. Commit-file rows paint the same trailing mark when that file has a line comment. `y` copies live comments as markdown after GC for the **focused** tree / graph / commit-file / diff row and descendants under that row (a file copies that path only; a folder copies files under it). Unrelated siblings stay out. The workspace row copies every live comment. Confirm-mode `y` is still revert-yes.

...
| `V`                                   | visual-line highlight on a focused file diff. `j` / `k` and arrows extend or shrink the range. `;` comments that line span. Esc leaves highlight without commenting. No-op on the tree, graph, or a commit-file list. Single-line `;` is unchanged when highlight is off |

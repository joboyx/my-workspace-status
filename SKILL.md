...
| `V`                                   | visual-line highlight on a focused file diff. `j` / `k` and arrows extend or shrink the range. `;` comments that line span. `s` / `u` stage / unstage the highlighted add/del lines. Esc leaves highlight without commenting. Watch reload, split/inline, or a painted-row change drops highlight. No-op on the tree, graph, or a commit-file list. |
| `y`                                   | copy comments for the focused row and its descendants as markdown (after GC) and show them in an overlay. A line comment with a newline stays one bullet. Confirm-mode `y` is still revert-yes |
...
| `m`                                   | toggle mouse reporting (on by default) except on a focused graph commit (merge, above). Drag pane / in-diff split / graph and file-diff h-bar thumbs |

...
describe('renderWorkspaceStatus', () => {
  ...
  it('renders Files header, 🔗 linked repo, merge marks, and Linked summary', () => {
    const snapshots: RepoSnapshot[] = [
      ...
      snap({
        repo: 'app/.worktrees/DEMO-1422-asr',
        branch: 'feature/DEMO-1422-asr',
        checkoutKind: 'linked',
        primaryRepo: 'app',
    ...
    assert.ok(
      lines.some((l) => l.includes('🔗 app/.worktrees/DEMO-1422-asr')),
      'expected 🔗 linked repo in verbose table',
    );
    assert.ok(
      lines.some((l) => l.includes('🚧 feature/DEMO-1422-asr 🌱')),
    ...
    assert.ok(
      lines.some((l) => l.includes('🔗 app/.worktrees/DEMO-1422-asr (DEMO-1422) 🌱')),
  ...
  it('keeps linked worktrees adjacent after primary in plain verbose buckets', () => {
    ...
    assert.deepEqual(
      ...
      ['acme-api', '🔗 acme-api/.worktrees/feat', 'acme-api-extra'],
    );
  });
  ...
  it('lists release and unknown non-default branches in the summary', () => {
    const snapshots: RepoSnapshot[] = [
      snap({
        ...
        branch: 'release/2026-07-20_DEMO2026053300',
      }),
      snap({
        repo: 'edge-proxy',
    ...
    assert.deepStrictEqual(
      ...
      [
        '🌿 Branches (2):',
        '  🚀 release:',
        '    - billing-service [release/2026-07-20_DEMO2026053300]',
        '  🌿 unknown:',

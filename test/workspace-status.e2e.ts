...
describe('workspace-status e2e', () => {
  ...
  it('branch-only summary lists clean non-default branch kinds', () => {
    ...
    createTrackingBranch('release-repo', 'release/2026-07-20_CR2026053300');
    ...
    assertLineMatches(
      output,
      /^release-repo\s+🚀\s+release\/2026-07-20_CR2026053300 🌱\s+✅ current\s+💾 clean/,
    );
    assertLineMatches(
      output,
    ...
    assertContains(output, '    - release-repo [release/2026-07-20_CR2026053300]');
    assertContains(output, '  🌿 unknown:');

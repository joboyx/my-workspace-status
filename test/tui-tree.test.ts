...
describe('buildTree + fold + flatten', () => {
  it('renders workspace root label in natural case', () => {
    ...
    const labelSeg = segs.find((s) => s.text === 'demo' || s.text === 'DEMO');
    assert.ok(labelSeg, 'expected a workspace label segment');
    assert.equal(labelSeg.text, 'demo');
    assert.notEqual(labelSeg.text, 'DEMO');
  });
  ...
  it('sorts linked worktrees adjacent after their primary', () => {
    ...
    const linkedOpen = base({
      repo: 'acme-api/.worktrees/feat-open',
      branch: 'feature/open',
      checkoutKind: 'linked',
      primaryRepo: 'acme-api',
      mergedIntoDefault: false,
      hasUnstaged: true,
      unstagedFiles: 'M\tsrc/b.ts',
    });
    const linkedMerged = base({
      repo: 'acme-api/.worktrees/feat-done',
      branch: 'feature/done',
      checkoutKind: 'linked',
      primaryRepo: 'acme-api',
    ...
    assert.deepEqual(topRepos, ['acme-api', 'web']);

    const family = tree.children.find((c) => c.kind === 'repo' && c.path === 'acme-api');
    ...
    assert.deepEqual(
      ...
      ['acme-api', 'acme-api/.worktrees/feat-done', 'acme-api/.worktrees/feat-open'],
    );
  });
  ...
  it('nests linked worktrees under a family container; flat when alone', () => {
    ...
    const family = nested.children.find((c) => c.kind === 'repo' && c.path === 'acme-api');
    assert.ok(family && family.kind === 'repo');
    ...
    const tiger = flat.children.find((c) => c.kind === 'repo' && c.path === 'web');
  ...
  it('linked checkout rows use linked icon, branch label, and merge mark', () => {
    ...
    const linked = base({
      repo: 'acme-api/.worktrees/DEMO-1422',
      branch: 'feature/DEMO-1422',
      checkoutKind: 'linked',
      primaryRepo: 'acme-api',
      mergedIntoDefault: false,
      hasUnstaged: true,
    ...
    const openRow = rows.find(
      (r) => r.node.kind === 'checkout' && r.node.path === 'acme-api/.worktrees/DEMO-1422',
    );
    ...
    assert.match(openRow.label, new RegExp(`feature\\/DEMO-1422 ${ICON_OPEN_VS_DEFAULT}`));
...
describe('status semantics — off-default / behind / ahead is not idle', () => {
  ...
  it('keeps clean main top-level when defaultBranchOverride is develop', () => {
    ...
    const topRepos = tree.children
      .filter((c) => c.kind === 'repo')
      .map((c) => (c.kind === 'repo' ? c.path : ''));
    assert.deepEqual(topRepos, ['acme-main']);
    const group = tree.children.find((c) => c.kind === 'group');
    assert.ok(group && group.kind === 'group');
    assert.equal(group.children.length, 1);
    assert.ok(group.children[0]?.kind === 'repo' && group.children[0].path === 'acme-frontend');
    assert.ok(
...
describe('clean-check gating', () => {
  ...
  it('omits the clean check on dirty family / checkout / worktree rows', () => {
    ...
    const rows = flatten(tree, createFoldState(tree));
    const family = rows.find((r) => r.node.kind === 'repo' && r.node.path === 'acme-api');

...
describe('linkedWorktreesUnderCwd', () => {
  ...
  it('remaps bind-mount aliases of the primary onto the cwd-visible path', () => {
    // Simulate: cwd sees primary at /ws/andromeda/dotfiles, but git registered
    // linked worktrees under /ws/dotfiles/... (same inode as andromeda/dotfiles).
    const ids = new Map<string, PathIdentity>([
      ['/ws/andromeda', { dev: 1, ino: 10 }],
      ['/ws/andromeda/dotfiles', { dev: 1, ino: 100 }],
      ['/ws/dotfiles', { dev: 1, ino: 100 }],
      ['/ws/dotfiles/.worktrees', { dev: 1, ino: 200 }],
      ['/ws/dotfiles/.worktrees/feat', { dev: 1, ino: 201 }],
      ['/ws/andromeda/dotfiles/.worktrees/feat', { dev: 1, ino: 201 }],
    ]);
    ...
    const entries = parseWorktreeListPorcelain(
      [
        'worktree /ws/andromeda/dotfiles',
        'branch refs/heads/main',
        '',
    ...
    const linked = linkedWorktreesUnderCwd(entries, '/ws/andromeda', '/ws/andromeda/dotfiles', io);
    assert.deepEqual(linked, [
      {
        absPath: '/ws/andromeda/dotfiles/.worktrees/feat',
  ...
  it('skips primary when git lists it under a bind-mount alias path', () => {
    ...
    assert.equal(
      mapLinkedWorktreeRelPath('/ws/dotfiles', '/ws/andromeda', '/ws/andromeda/dotfiles', io),
      null,
    );
  });
  ...
  it('prefers cwd-visible primary prefix when alias path is also under cwd', () => {
    // cwd=/ws contains both andromeda/dotfiles (primary) and sibling bind-mount
    ...
    const mapped = mapLinkedWorktreeRelPath(
      ...
      '/ws',
      '/ws/andromeda/dotfiles',
      io,
    );
    assert.deepEqual(mapped, {
      absPath: '/ws/andromeda/dotfiles/.worktrees/feat',
      relPath: 'andromeda/dotfiles/.worktrees/feat',
    });
  });
});
...
describe('resolveWorktreeRemoveTarget', () => {
  it('rewrites bind-mount aliases to git-registered path and cwd', () => {
    // TUI sees andromeda/dotfiles/...; git registered personal/dotfiles/...
    const ids = new Map<string, PathIdentity>([
      ['/ws/andromeda/dotfiles', { dev: 1, ino: 100 }],
    ...
    assert.deepEqual(
      resolveWorktreeRemoveTarget(
        entries,
        '/ws/andromeda/dotfiles',
        '/ws/andromeda/dotfiles/.worktrees/feat',
        io,
      ),
      {

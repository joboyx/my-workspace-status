...
describe('viewed store path + load/save', () => {
  it('prefers WS_STATUS_VIEWED_STORE then XDG_STATE_HOME', () => {
    ...
    assert.equal(
      viewedStorePath({ XDG_STATE_HOME: '/xdg/state', HOME: '/home/demo' }),
      path.join('/xdg/state', 'my-workspace-status', 'viewed-files.json'),
    );
    assert.equal(
      viewedStorePath({ HOME: '/home/demo' }),
      path.join('/home/demo', '.local', 'state', 'my-workspace-status', 'viewed-files.json'),
    );
  });

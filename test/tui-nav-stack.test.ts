...
describe('applyNavEnter', () => {
  ...
  it('right + Enter at depth 0 pushes repoGraph and stays on right', () => {
    ...
    nav = applyNavEnter(nav, drill({ repo: 'recorded-services', commitId: null }));
  ...
  it('right + Enter at depth 1 pushes commitFiles and stays on right', () => {
    let nav: NavState = {
      stack: [
        { kind: 'workspace' },
        { kind: 'repoGraph', repo: 'recorded-services', commitId: 'a1b2c3d' },
      ],
      focusPane: 'right',
    };
  ...
  it('right + Enter at depth 1 with null commitId uses WORKTREE stub id', () => {
    ...
    nav = applyNavEnter(nav, drill({ repo: 'rs', commitId: null }));
    assert.deepEqual(currentView(nav), {
      kind: 'commitFiles',
      repo: 'rs',
      commitId: 'WORKTREE',
  ...
  it('right + Enter at depth 1 prefers drill.commitId over null stack commitId', () => {
    ...
    nav = applyNavEnter(nav, drill({ repo: 'rs', commitId: 'deadbeefcafe' }));
    assert.deepEqual(currentView(nav), {
      kind: 'commitFiles',
      repo: 'rs',
  ...
  it('right + Enter at depth 1 prefers drill.commitId over stack commitId', () => {
    let nav: NavState = {
      stack: [
        { kind: 'workspace' },
        { kind: 'repoGraph', repo: 'rs', commitId: 'oldcommit' },
      ],
      focusPane: 'right',
    };
    nav = applyNavEnter(nav, drill({ repo: 'rs', commitId: 'newcommit' }));
    assert.deepEqual(currentView(nav), {
      kind: 'commitFiles',
      repo: 'rs',
      commitId: 'newcommit',
...
describe('breadcrumbSegments / formatBreadcrumb', () => {
  it('mirrors the stack and focus pane', () => {
    const nav: NavState = {
      stack: [
        { kind: 'workspace' },
        { kind: 'repoGraph', repo: '/ws/recorded-services', commitId: 'a1b2c3d4e5' },
        {
          kind: 'commitFiles',
          repo: '/ws/recorded-services',
          commitId: 'a1b2c3d4e5',
          filePath: 'src/fetch.ts',
        },
      ],
    ...
    assert.deepEqual(breadcrumbSegments(nav, 'andromeda'), [
      'andromeda',
      'recorded-services',
      'a1b2c3d',
      'fetch.ts',
    ...
    assert.equal(
      formatBreadcrumb(breadcrumbSegments(nav, 'andromeda'), 'right'),
      'andromeda › recorded-services › a1b2c3d › fetch.ts · right',
    );
  });
  ...
  it('drops trailing segments on pop', () => {
    ...
    assert.deepEqual(breadcrumbSegments(after, 'andromeda'), ['andromeda']);
  });
  ...
  it('shows uncommitted for WORKTREE without shortHash truncation', () => {
    const nav: NavState = {
      stack: [
        { kind: 'workspace' },
        { kind: 'repoGraph', repo: 'rs', commitId: null },
        { kind: 'commitFiles', repo: 'rs', commitId: 'WORKTREE', filePath: null },
      ],
      focusPane: 'left',
    };
    assert.deepEqual(breadcrumbSegments(nav, 'andromeda'), [

...
describe('buildCommitFileNodes', () => {
  it('tree mode nests dirs', () => {
    const nodes = buildCommitFileNodes('rs', changes, true);
  ...
  it('flat mode is file-only list', () => {
    const nodes = buildCommitFileNodes('rs', changes, false);
...
describe('unfoldForestAncestors', () => {
  it('opens folded parent dirs so a nested file is visible', () => {
    const nodes = buildCommitFileNodes('rs', changes, true);
    const folds = new Set(['dir:rs:src']);
    const hidden = flattenCommitFiles(nodes, folds, true);
    assert.ok(!hidden.some((r) => r.id === 'file:rs:src/a.ts'));

    const revealed = unfoldForestAncestors(nodes, folds, 'file:rs:src/a.ts');
    assert.ok(!revealed.has('dir:rs:src'));
    const visible = flattenCommitFiles(nodes, revealed, true);
    assert.ok(visible.some((r) => r.id === 'file:rs:src/a.ts'));
  });

  it('returns the same set when the id is missing', () => {
    const nodes = buildCommitFileNodes('rs', changes, true);
    const folds = new Set(['dir:rs:src']);
    assert.equal(unfoldForestAncestors(nodes, folds, 'file:rs:gone.ts'), folds);
  });

  it('restores list focus onto a file that was hidden under a fold', () => {
    const nodes = buildCommitFileNodes('rs', changes, true);
    const folds = new Set(['dir:rs:src']);
    const nextFolds = unfoldForestAncestors(nodes, folds, 'file:rs:src/util/b.ts');
    const painted = flattenCommitFiles(nodes, nextFolds, true);
    const restored = resolveListFocus(painted, 'file:rs:src/util/b.ts', 0);
    assert.equal(painted[restored.cursor]?.id, 'file:rs:src/util/b.ts');
    assert.equal(restored.focusId, 'file:rs:src/util/b.ts');
  });
});

...
use crate::seed::{
    ahead_workspace, daily_workspace, focus_workspace, seed_long_diff_file, seed_long_path_file,
    seed_repo, stream_workspace, unfetched_behind_workspace, worktree_workspace,
};
...
fn syncbox_row_current(screen: &str) -> bool {
    after_syncbox_name(screen).is_some_and(|after| {
        after.contains("& main") && after.contains('.') && !after.contains("^1")
    })
}

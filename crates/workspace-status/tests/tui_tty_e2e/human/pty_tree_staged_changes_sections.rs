...
/// ASCII PTY: `# Staged` then `~ Changes` on the left tree.
...
/// PTY (`WS_STATUS_GLYPHS=ascii`): left tree after first paint.
...
fn pty_tree_unstaged_only_omits_staged_changes_sections() {
    ...
    tui.wait_pred(
        ...
        "first paint: left tree has no Staged / Changes section labels",
        WAIT,
    );
}
...
///
/// Docs: any staged path → `# Staged` then `~ Changes` (ASCII PTY).
...
fn pty_tree_staged_and_changes_paint_section_headers() {
    ...
    tui.wait_pred(
        ...
        "first paint: left tree shows ASCII # Staged then ~ Changes",
        WAIT,
    );
}
